# -*- coding: utf-8 -*-
"""
Created on Wed Oct 15 11:44:08 2025
...
start = pd.Timestamp("2025-08-10 11:37:28")
end = pd.Timestamp("2025-08-10 12:26:06")
...
volume_flow_N2_mins = T_window.values[200][0] #Ln/min 
...
volume_flow_CO2_mins1 = T_window.values[100][2] #Ln/min
volume_flow_CO2_mins2 = T_window.values[200][2] #Ln/min
volume_flow_CO2_sec1 = volume_flow_CO2_mins1 / 60 #Ln/sec
volume_flow_CO2_sec2 = volume_flow_CO2_mins2 / 60 #Ln/sec
thermal_mass_flow_CO21 = heat_capacity_CO2 * density_CO2 * volume_flow_CO2_sec1 #W/K
thermal_mass_flow_CO22 = heat_capacity_CO2 * density_CO2 * volume_flow_CO2_sec2 #W/K


thermal_mass_flow_combined1 = thermal_mass_flow_CO21 + thermal_mass_flow_N2 #W/K
thermal_mass_flow_combined2 = thermal_mass_flow_CO22 + thermal_mass_flow_N2 #W/K

def CO2flow(t):
    if t < 156:
        CO2flow = volume_flow_CO2_sec1
    else:
        CO2flow = volume_flow_CO2_sec2
    return CO2flow    

def sublimation_heat_CO2(volume_flow_CO2):
    sub_heat = enthalpy_of_sublimation_CO2 * volume_flow_CO2 * density_CO2 #W
    return sub_heat
...
partial_pressure_CO2 = volume_flow_CO2_sec2/(volume_flow_CO2_sec2 + volume_flow_N2_sec)
...
def heat_and_massbalance(t, y, params):
 ...
 #   first_tank_surface = 0.005 #m^2
    dTdt[0] = (tank_surface*alpha*(T_outside - T[0]) + Boltzman_constant*epsilon*(T_outside**4 - T[0]**4) + alpha_conv*(T_in - T[0]))/(first_tank_volume*density_glass*heat_capacity_glass/trunk)
    F[0] = CO2flow(t)
    # Trunk tanks
    for i in range(1, trunk):        
        ...
        heat_conv = alpha_conv*(T[i-1] - T[i])*(heat_capacity_N2*density_N2*volume_flow_N2_sec + 
                                                 heat_capacity_CO2*density_CO2*F[i-1])
        ...
        #predesublimation
            ...
            dmdt[i] = 0
            F[i] = F[i-1] - dmdt[i] / density_CO2
            dTdt[i] = (heat_env + heat_conv + heat_desub) / (first_tank_volume*density_glass*heat_capacity_glass/trunk + m[i]*heat_capacity_CO2)
        #sublimation
    ...
    for i in range(trunk, T.size):
        ...
        #sublimation
            ...
            dTdt[i] = (heat_env + heat_conv + heat_sub) / (tank_volume*density_glass*heat_capacity_glass + m[i]*heat_capacity_CO2)  #K/s
        #post-desublimation
        else:
            dmdt[i] = 0
            F[i] = F[i-1] - dmdt[i] / density_CO2
...
def reconstruct_flowrate(t, m_sim, rho_CO2):
    ...
    F[:200, 0] = volume_flow_CO2_sec1
    F[200:, 0] = volume_flow_CO2_sec2
...
# PCHIP (monotone cubic) — prevents overshoot and waviness
...
# generate a curve with fitted parameters
...
# Last subplot for total mass and legend
ax11 = axes[11]
...
#Condensed plot of three tanks
for i in range(1,7,11):
    plt.plot(t_span, T_sim_meas[:, [i]], label="T_model", color='b', alpha=(0.5+i/22))
    plt.plot(t_span, T_interpol[:, [i]], label="T_measured", color='c', alpha=(0.5+i/22))
plt.xlabel("Time [s]")
plt.ylabel("Temperature [K]")
