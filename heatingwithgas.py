# -*- coding: utf-8 -*-
"""
Created on Wed Sep 24 10:56:06 2025
...
path = r"C:\Users\20223544\TU Eindhoven\Vullings, Stan - PPT\Data\Day 7\day 7 - heating with n2 flow.csv"
...
Thermal_mass_flow_no_denisty = heat_capacity_CO2 * volume_flow_CO2_sec + heat_capacity_N2 * volume_flow_N2_sec #Wkg/km^3
...
def heatbalance(T, t_span, params):
    alpha_conv, T_in, Rho_gas = params
    dTdt = np.zeros_like(T)
    #first tank
    first_tank_volume = 150 * 10**-3 # L; measured as the volume of the zone before the beads
 #   first_tank_surface = 0.005 #m^2
    dTdt[0] = (tank_surface*alpha*(T_outside - T[0]) + Boltzman_constant*epsilon*(T_outside**4 - T[0]**4) + alpha_conv*(T_in - T[0]))/(first_tank_volume/trunk*density_glass*heat_capacity_glass) + Thermal_mass_flow_no_denisty*Rho_gas*(T_in -T[i])
    for i in range (1, trunk):
        dTdt[i] = (tank_surface*alpha*(T_outside - T[i]) + Boltzman_constant*epsilon*(T_outside**4 - T[i]**4) + alpha_conv*(T[i-1] - T[i]))/(first_tank_volume*tank_volume*density_glass*heat_capacity_glass/trunk) + Thermal_mass_flow_no_denisty*Rho_gas*(T[i-1] -T[i])
    for i in range(trunk, T.size):
        dTdt[i] = (tank_surface*alpha*(T_outside - T[i]) + Boltzman_constant*epsilon*(T_outside**4 - T[i]**4) + alpha_conv*(T[i-1] - T[i]))/(tank_volume*density_glass*heat_capacity_glass) + Thermal_mass_flow_no_denisty*Rho_gas*(T[i-1] -T[i])
    return dTdt
...
T_in0 = 10
Rho_gass0 = 4
x0 = np.array([alpha0, T_in0, Rho_gass0])
lower = [0,         -10, 0]
upper = [20000.0,    20, 0]
...
params_fit = res.x
print("Fitted parameters alpha_conv, T_in=, Rho_gas=", params_fit)
