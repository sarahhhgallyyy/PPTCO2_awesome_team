...
# plt.plot(t_span[:int(t_span.size*.6)], F_sim[:int(t_span.size*.6), [1,4,8,12]], label="CO₂ flow per tank")
# plt.xlabel("Time [s]")
# plt.ylabel("CO₂ flowrate [Ln/s]")
# plt.title("0.167 Ln/s inflow with step, 12 tanks")
# plt.legend(["tank 1", "tank 4", "tank 8", "tank 12"])
# plt.show()
# kl

#from Model1 import temperature_steady_state
path = r"Data\Step function from 2.5 CO2 17.5 N2 to 10 10.csv"
...
start = pd.Timestamp("2025-09-24 11:25:49")
end = pd.Timestamp("2025-09-24 12:15:04")
 ...
 #   first_tank_surface = 0.005 #m^2
    ...
    for i in range(1, trunk):        
        ...
        #predesublimation
            ...
            dmdt[i] = 0
            F[i] = F[i-1] - dmdt[i] / density_CO2
            dTdt[i] = (heat_env + heat_conv + heat_desub) / (first_tank_volume*density_glass*heat_capacity_glass*Factor_packing/trunk + m[i]*heat_capacity_CO2)
        #sublimation
    ...
    for i in range(trunk, T.size):
        ...
        #     CO2_flowrate[i] = CO2_flowrate[i-1] - (dmdt[i] / density_CO2) #Ln/s
            ...
            dTdt[i] = (heat_env + heat_conv + heat_sub) / (tank_volume*density_glass*heat_capacity_glass*Factor_packing + m[i]*heat_capacity_CO2)  #K/s
        #post-desublimation
        else:
            dmdt[i] = 0
            F[i] = F[i-1] - dmdt[i] / density_CO2
